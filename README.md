...
- **Blocker defaults, worth knowing:** out of the box `Schedule` is ON (Mon–Fri 09:00–18:00) and the Focus Pack contains `facebook.com` and `youtube.com`, so a *fresh* install blocks both during work hours even though the panel's `Focus` toggle reads OFF — the schedule gates the same filter independently. Turn `Schedule` off in the Blocker section, or add the sites to `Allowed`, if that is not what you want. Existing installs keep their saved settings and are unaffected.
