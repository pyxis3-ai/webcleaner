...
- **Confirmed on a live logged-in feed: the 8.8.1 mobile matcher fix was both real and necessary.** Over 289 post containers the desktop matcher hid 76 and the old mobile matcher hid **0 of those 76**; the new one hides all 76, exactly matching desktop. The cause is visible in the captured headers — the label always trails the page name, as in `Haven House Children's Hospice · Sponsored` and `The Queens · Sponsored` — so the old `startsWith` test could never fire.
