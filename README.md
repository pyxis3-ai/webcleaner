...
- **Fixed: ad skipping never ran on mobile.** Mobile YouTube loads a different player binary — `player-plasma-es6` rather than desktop's `player_ias`. Of the eight `.ytp-ad-*` overlay classes the ad-UI test looked for, **all eight are absent** from the mobile player, so `adUiPresent` stayed false there. The gate was `(adClass || adApi) && adUiPresent`, so on mobile it could only fire once a skip button had already rendered — the unskippable opening seconds were never detected, `Mute ads` never engaged, and unskippable ads were never seeked past. Detection now trusts `getPresentingPlayerType()` on its own, present 47 times in the mobile player and returning 1 for ordinary playback on both platforms; the older class path keeps its UI confirmation. The same one-sided gate is fixed for Shorts. (An earlier draft of this note also claimed `ad-showing`/`ad-interrupting` never appear on mobile. Those strings are absent from the mobile *binary*, but the classes are applied to the player at runtime — a captured live mobile ad shows `cls: true`. The half that actually failed was the ad-UI test, not the class test.)
