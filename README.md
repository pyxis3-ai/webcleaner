...
- **Focus Pack is opt-in.** `Focus` and `Schedule` are both OFF by default, so nothing in the Focus Pack is blocked until you turn one on. Two independent switches feed the same filter — `Focus` blocks the pack always, `Schedule` blocks it only inside the configured window — so the Blocker section shows a live **effective state** line telling you whether the pack is blocking *right now* and why (`● Focus pack BLOCKING NOW (inside schedule 09:00–18:00)` / `○ Focus pack not blocking (outside schedule …)`), and flags when the site you are on is one of them. **If you upgraded from an earlier version your saved settings are kept**, so if `Schedule` was on it stays on — the state line will now make that visible.
