...
(function () {
  ...
  const txtRow = (l, m, k) =>
    `<div class="fr"><span style="font-size:12px;color:#8a8a90">${esc(l)}</span><input class="tx" type="text" autocorrect="off" autocapitalize="none" data-txt="${m}.${k}" value="${esc(C[m][k])}"></div>`;
  const hkRow  = (m) =>
  ...
  function listBlock(label, mod, key, ph) {
    ...
    return `<div class="sc"><h2>${esc(label)}</h2>${items}
      <div class="ad"><input type="text" autocorrect="off" autocapitalize="none" data-ai="${path}" placeholder="${esc(ph)}"><button data-ab="${path}">+</button></div></div>`;
  }
  ...
  const secSB = () => {
    ...
    return `<details data-s=sb open>
      ...
      <details data-s=sb-adv><summary>Advanced</summary>
        ${time2("From","siteBlocker","from","To","to")}
        ${numRow("Snooze minutes","siteBlocker","snoozeMinutes")}
        ${hkRow("siteBlocker")}
      </details>
    </details>`;
  };
  ...
  const secVM = () => {
    ...
    return `<details data-s=vm>
      ...
      <div class="r2"><span>Site</span><div class="sg">${modes.map((m) => seg(m,"data-vm")).join("")}</div>
        <span style="margin-left:auto">Default</span><div class="sg">${modes.map((m) => seg(m,"data-df")).join("")}</div></div>
  ...
  const secFB = () => `<details data-s=facebook>
    ...
    ${swGrid("facebook",[
      ["Enabled","enabled"],["Sponsored","hideSponsored"],["Suggested","hideSuggested"],
      ["People YMKN","hidePeopleYouMayKnow"],["Reels/Stories","hideReelsTrays"],["Right sidebar","hideRightSidebar"],
      ["Left sidebar","hideLeftSidebar"],["Composer","hideComposer"],["Top bar","hideTopBar"],
      ["Strip tracking","stripTracking"],["Skip Reel ads","skipReelsAds"],["Most Recent","forceMostRecent"],
    ])}
  ...
  const secYT = () => `<details data-s=youtube>
    ...
    ${swGrid("youtube",[
      ...
      ["Anti-adblock","dismissAntiAdblock"],["Show button","showToggleButton"],
    ])}
