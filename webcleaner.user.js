...
(function () {
  ...
  const isIG  = /^(www\.)?instagram\.com$/.test(HOST);
  const isX   = /^(www\.)?x\.com$|^(www\.)?twitter\.com$/.test(HOST);
  const isRD  = /^(www\.)?reddit\.com$/.test(HOST);
  ...
  const DEF = {
    ...
    youtube: {
      ...
      hideEndCards:false, hideInfoCards:false, hideAutoplay:false,
      showToggleButton:true,
      toggleHotkey:{ctrl:false,alt:true,shift:true,key:"y"},
    },
    instagram: {
      enabled:true, hideSponsored:true, hideSuggested:true, hideReels:false,
      hideStories:false, showToggleButton:true,
      toggleHotkey:{ctrl:false,alt:true,shift:true,key:"i"},
    },
    twitter: {
      enabled:true, hidePromoted:true, hideSuggested:true, hideTrends:true,
      hideFollowSuggestions:true, showToggleButton:true,
      toggleHotkey:{ctrl:false,alt:true,shift:true,key:"t"},
    },
    reddit: {
      enabled:true, hidePromoted:true, hideRecommended:true, showToggleButton:true,
      toggleHotkey:{ctrl:false,alt:true,shift:true,key:"r"},
    },
    siteBlocker: {
  ...
  // Trusted Types safe innerHTML — only used for shadow root
  ...
  // Single global hotkey listener — avoids stacking multiple listeners
  const _hotkeys=[];
  ...
  // Single interceptNav call — prevents double-wrapping
  let _navWrapped=false;
  const _navCbs=[];
  function interceptNav(cb){ _navCbs.push(cb); if(_navWrapped) return; _navWrapped=true; const w=orig=>function(...a){let r;try{r=orig.apply(this,a);}catch(e){throw e;} _navCbs.forEach(fn=>{try{fn();}catch(_){}});return r;}; try{history.pushState=w(history.pushState);}catch(_){} try{history.replaceState=w(history.replaceState);}catch(_){} window.addEventListener("popstate",()=>_navCbs.forEach(fn=>{try{fn();}catch(_){};})); }
  ...
  const secFB=()=>`<details data-s=facebook><summary>🧹 FB ${C.facebook.enabled?"ON":"OFF"}</summary>${swG("facebook",[["On","enabled"],["Sponsored","hideSponsored"],["Suggested","hideSuggested"],["People YMKN","hidePeopleYouMayKnow"],["Reels/Stories","hideReelsTrays"],["Comments","hideComments"],["Video autoplay","hideVideoAutoplay"],["Like counts","hideLikeCounts"],["R.sidebar","hideRightSidebar"],["L.sidebar","hideLeftSidebar"],["Composer","hideComposer"],["Top bar","hideTopBar"],["Tracking","stripTracking"],["Reel ads","skipReelsAds"],["Most Recent","forceMostRecent"],["Button","showToggleButton"]])}${listBlock("Junk phrases","facebook","extraJunkPhrases","phrase")}<details data-s=fb-adv><summary>Advanced</summary>${hkR("facebook")}</details></details>`;

  const secYT=()=>`<details data-s=youtube><summary>⏭ YT ${C.youtube.enabled?"ON":"OFF"}</summary>${swG("youtube",[["On","enabled"],["Video ads","skipVideoAds"],["Shorts ads","skipShortsAds"],["Feed ads","hideFeedAds"],["Banners","hideBanners"],["Mute ads","muteAds"],["Anti-AB","dismissAntiAdblock"],["Hide Shorts","hideShorts"],["End cards","hideEndCards"],["Info cards","hideInfoCards"],["Autoplay","hideAutoplay"],["Button","showToggleButton"]])}<details data-s=yt-adv><summary>Advanced</summary>${hkR("youtube")}</details></details>`;

  const secIG=()=>`<details data-s=instagram><summary>📷 IG ${C.instagram.enabled?"ON":"OFF"}</summary>${swG("instagram",[["On","enabled"],["Sponsored","hideSponsored"],["Suggested","hideSuggested"],["Hide Reels","hideReels"],["Hide Stories","hideStories"],["Button","showToggleButton"]])}<details data-s=ig-adv><summary>Advanced</summary>${hkR("instagram")}</details></details>`;

  const secTW=()=>`<details data-s=twitter><summary>🐦 X/Twitter ${C.twitter.enabled?"ON":"OFF"}</summary>${swG("twitter",[["On","enabled"],["Promoted","hidePromoted"],["Suggested","hideSuggested"],["Trends","hideTrends"],["Follow suggestions","hideFollowSuggestions"],["Button","showToggleButton"]])}<details data-s=tw-adv><summary>Advanced</summary>${hkR("twitter")}</details></details>`;

  const secRD=()=>`<details data-s=reddit><summary>🟠 Reddit ${C.reddit.enabled?"ON":"OFF"}</summary>${swG("reddit",[["On","enabled"],["Promoted","hidePromoted"],["Recommended","hideRecommended"],["Button","showToggleButton"]])}<details data-s=rd-adv><summary>Advanced</summary>${hkR("reddit")}</details></details>`;
  ...
  const Panel=(()=>{
    ...
    const affects=m=>m==="siteBlocker"?"block":m==="facebook"?isFB:m==="youtube"?isYT:m==="instagram"?isIG:m==="twitter"?isX:m==="reddit"?isRD:m==="viewMode"?vmActive():false;
    ...
    function render(){
      ...
      safeHTML(sh,`<style>${PCSS}</style><div class="bk" data-x></div><div class="cd" role="dialog"><div class="hd"><h1>🧼 Web Cleaner</h1><button class="x" data-x>✕</button></div>${secSB()}${secVM()}${isFB?secFB():""}${isYT?secYT():""}${isIG?secIG():""}${isX?secTW():""}${isRD?secRD():""}${secIO()}</div>`);
    ...
    function wire(sh){
      ...
      qa("[data-sw]",sh).forEach(e=>e.addEventListener("change",()=>{
        ...
        if(m==="instagram"&&k==="enabled"){toggleIG(e.checked);render();return;}
        if(m==="twitter"&&k==="enabled"){toggleTW(e.checked);render();return;}
        if(m==="reddit"&&k==="enabled"){toggleRD(e.checked);render();return;}
  ...
  function initCluster(){
    ...
    if(isIG&&C.instagram.showToggleButton) addBtn("📷","#fff","#111",C.instagram.enabled?"1":".3","ig-btn").addEventListener("click",()=>{if(!drag)toggleIG(!C.instagram.enabled);});
    if(isX&&C.twitter.showToggleButton)    addBtn("🐦","#fff","#111",C.twitter.enabled?"1":".3","tw-btn").addEventListener("click",()=>{if(!drag)toggleTW(!C.twitter.enabled);});
    if(isRD&&C.reddit.showToggleButton)    addBtn("🟠","#fff","#111",C.reddit.enabled?"1":".3","rd-btn").addEventListener("click",()=>{if(!drag)toggleRD(!C.reddit.enabled);});
  ...
  function initFB(){
    ...
    (()=>{
      ...
      if(!isMFB){
        ...
        if(f.hideComments)R.push(`${X}[role="article"] [aria-label="Comment"],[role="article"] [aria-label^="Comments"]{display:none!important}`);
      ...
      // Block video autoplay via attribute override
      if(f.hideVideoAutoplay){
        const muteVids=()=>{for(const v of document.querySelectorAll("video:not([data-fcf-muted])")){v.setAttribute("data-fcf-muted","");v.muted=true;v.autoplay=false;try{v.pause();}catch(_){}}};
        onReady(muteVids);new MutationObserver(muteVids).observe(document.documentElement,{childList:true,subtree:true});
      }
    })();
    ...
    function feedBox(){
      ...
      let best=null,bn=1;
      // Use children of main's first-level divs only — avoids O(n²) deep scan
      for(const d of main.children){
        let n=0;
        for(const c of d.children){const r=c.getBoundingClientRect();if(r.width>=500&&r.width<=720&&r.height>60)n++;}
        if(n>bn){bn=n;best=d;}
        // Also check one level deeper
        for(const d2 of d.children){
          let n2=0;
          for(const c of d2.children){const r=c.getBoundingClientRect();if(r.width>=500&&r.width<=720&&r.height>60)n2++;}
          if(n2>bn){bn=n2;best=d2;}
        }
      }
      return(_feed=best);
    }
  ...
  function initYT(){
    ...
    if(y.hideShorts)rules.push("ytd-rich-section-renderer:has(ytd-reel-shelf-renderer)","ytd-reel-shelf-renderer","a[href^='/shorts']");
    if(y.hideEndCards)rules.push(".ytp-ce-element",".ytp-endscreen-content");
    if(y.hideInfoCards)rules.push(".ytp-cards-teaser",".ytp-card-content");
    ...
    // Autoplay: disable via attribute
    if(y.hideAutoplay){
    ...
    // hideFeedAds: run on mutation only, not on every tick
  ...
  // ── instagram ──────────────────────────────────────────────
  const toggleIG=on=>{C.instagram.enabled=on;save("instagram");const b=document.getElementById("ig-btn");if(b)b.style.opacity=on?"1":".3";applyIGStyle();};

  function applyIGStyle(){
    const ig=C.instagram;
    const rules=[];
    if(ig.enabled){
      if(ig.hideSponsored)rules.push('article:has([data-testid="post-badge"])','[data-testid="sponsored-marker"]');
      if(ig.hideSuggested)rules.push('div[data-testid="suggested-posts"]','._aatb');
      if(ig.hideReels)rules.push('a[href*="/reels/"]>div','[aria-label="Reels"]');
      if(ig.hideStories)rules.push('section [role="menuitem"]>div[style*="border-radius: 50%"]');
    }
    if(!document.getElementById("ig-css")&&rules.length){addStyle("ig-css",rules.join(",")+"{display:none!important}");}
    else if(document.getElementById("ig-css")){document.getElementById("ig-css").disabled=!ig.enabled||!rules.length;}
  }

  function initIG(){
    applyIGStyle();
    onHotkey(()=>C.instagram.toggleHotkey,()=>toggleIG(!C.instagram.enabled));
  }

  // ── twitter / x ────────────────────────────────────────────
  const toggleTW=on=>{C.twitter.enabled=on;save("twitter");const b=document.getElementById("tw-btn");if(b)b.style.opacity=on?"1":".3";applyTWStyle();};

  function applyTWStyle(){
    const tw=C.twitter;
    const rules=[];
    if(tw.enabled){
      if(tw.hidePromoted)rules.push('[data-testid="placementTracking"]');
      if(tw.hideSuggested)rules.push('[aria-label="Who to follow"]','[data-testid="UserCell"]');
      if(tw.hideTrends)rules.push('[aria-label="Trending"]','[data-testid="trend"]');
      if(tw.hideFollowSuggestions)rules.push('[data-testid="sidebarColumn"] [data-testid="UserCell"]');
    }
    addStyle("tw-css",rules.length?rules.join(",")+"{display:none!important}":"");
  }

  function initTW(){
    applyTWStyle();
    onHotkey(()=>C.twitter.toggleHotkey,()=>toggleTW(!C.twitter.enabled));
  }

  // ── reddit ─────────────────────────────────────────────────
  const toggleRD=on=>{C.reddit.enabled=on;save("reddit");const b=document.getElementById("rd-btn");if(b)b.style.opacity=on?"1":".3";applyRDStyle();};

  function applyRDStyle(){
    const rd=C.reddit;
    const rules=[];
    if(rd.enabled){
      if(rd.hidePromoted)rules.push('[data-promoted="true"]','shreddit-ad-post','[data-adclick-id]');
      if(rd.hideRecommended)rules.push('shreddit-feed-unit-container');
    }
    addStyle("rd-css",rules.length?rules.join(",")+"{display:none!important}":"");
  }

  function initRD(){
    applyRDStyle();
    onHotkey(()=>C.reddit.toggleHotkey,()=>toggleRD(!C.reddit.enabled));
  }
  ...
  function regMenu(){
    ...
    if(isIG)GM_registerMenuCommand(`📷 IG ${C.instagram.enabled?"ON":"OFF"}`,()=>toggleIG(!C.instagram.enabled));
    if(isX) GM_registerMenuCommand(`🐦 X ${C.twitter.enabled?"ON":"OFF"}`,()=>toggleTW(!C.twitter.enabled));
    if(isRD)GM_registerMenuCommand(`🟠 Reddit ${C.reddit.enabled?"ON":"OFF"}`,()=>toggleRD(!C.reddit.enabled));
