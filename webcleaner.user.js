...
(function () {
  ...
  const BOUNDS = { snoozeMinutes:[1,1440], desktopWidth:[320,7680], mobileWidth:[240,1080], mobileHeight:[400,2400], mobileDpr:[0.5,5], longPressMs:[100,5000] };

  const DEF = {
    facebook:    { enabled:true, hideSponsored:true, hideSuggested:true, hidePeopleYouMayKnow:true, hideReelsTrays:true, stripTracking:true, showToggleButton:true, hideRightSidebar:true, hideLeftSidebar:true, hideComposer:true, hideTopBar:true, skipReelsAds:true, forceMostRecent:true, extraJunkPhrases:[], toggleHotkey:{ctrl:false,alt:true,shift:true,key:"f"} },
    youtube:     { enabled:true, skipVideoAds:true, skipShortsAds:true, hideFeedAds:true, hideBanners:true, muteAds:true, dismissAntiAdblock:true, showToggleButton:true, toggleHotkey:{ctrl:false,alt:true,shift:true,key:"y"} },
    siteBlocker: { enabled:true, blockAdult:true, blockFocus:false, scheduleOn:true, snoozeMinutes:5, schedule:{days:[1,2,3,4,5],from:"09:00",to:"18:00"}, custom:[], allow:[], toggleHotkey:{ctrl:false,alt:true,shift:true,key:"b"} },
    viewMode:    { newSiteDefault:"auto", showButton:true, spoofUA:true, spoofTouch:true, spoofMedia:true, frameOnDesktop:false, longPressMs:500, desktopWidth:1280, mobileWidth:412, mobileHeight:915, mobileDpr:2.625, mobileUA:"Mozilla/5.0 (Linux; Android 14; Pixel 8) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Mobile Safari/537.36", desktopUA:"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36", toggleHotkey:{ctrl:false,alt:true,shift:true,key:"v"} },
  };
  ...
  function deepMerge(d, o) {
    ...
    for (const k of Object.keys(o))
      r[k] = (k in r && !Array.isArray(r[k]) && typeof r[k]==="object" && !Array.isArray(o[k]) && typeof o[k]==="object")
        ? deepMerge(r[k], o[k]) : o[k];
    return r;
  }

  const C = {}; for (const m of Object.keys(DEF)) C[m] = deepMerge(DEF[m], gGet(m, null));
  ...
  // addStyle: always uses textContent (never innerHTML) — CSP-safe on all sites
  const addStyle = (id, css, root) => {
    const p = root || document.head || document.documentElement;
    if (p.querySelector?.(`#${id}`)) return;
    const s = mk("style", {id});
    s.textContent = css;   // textContent is NOT a Trusted Types sink — always safe
    p.appendChild(s);
  };

  // safeSetHTML: sets innerHTML on a shadow root using Trusted Types policy if available.
  // Shadow DOM innerHTML is the ONLY place we use innerHTML — everything else uses
  // textContent or DOM construction.
  const safeSetHTML = (() => {
    // Try to get or create a Trusted Types policy.
    // On YouTube this succeeds; on other sites trustedTypes may not exist.
    const getPolicy = () => {
      try {
        if (typeof trustedTypes === "undefined") return null;
        // Re-use an existing policy if we already created one
        if (trustedTypes.defaultPolicy) return null; // can't create named if default exists
        return trustedTypes.createPolicy("wc-html", { createHTML: s => s });
      } catch(_) { return null; }
    };
    const pol = getPolicy();
    return (el, html) => {
      try {
        ...
        // DOMParser fallback: parses HTML into a temporary document,
        // then transplants child nodes. Does NOT inject any script tag.
        try {
          const tmp = new DOMParser().parseFromString(
            `<!DOCTYPE html><html><head></head><body>${html}</body></html>`, "text/html"
          );
          el.textContent = "";
          // Move <style> elements from <head> first
          for (const n of Array.from(tmp.head.childNodes)) el.appendChild(document.adoptNode(n));
          for (const n of Array.from(tmp.body.childNodes)) el.appendChild(document.adoptNode(n));
        } catch(_2) {}
      }
    };
  })();
  ...
  // interceptNav: wraps history API for SPA navigation detection.
  // Uses plain function wrapping — no eval, no script tags, no Trusted Types sinks.
  function interceptNav(cb) {
    const wrap = orig => function(...a) {
      let r; try { r = orig.apply(this, a); } catch(e) { throw e; }
  ...
  // defProp: Object.defineProperty from userscript sandbox affects page window.
  // No script injection — CSP-safe.
  const defProp = (obj, key, get) => {
    try { Object.defineProperty(obj, key, {configurable:true, get}); } catch(_) {}
  };

  // ── view mode spoof (runs at document-start, before any page JS) ──
  function applyVMSpoof() {
    ...
    if (v.spoofMedia) {
      ...
      const nat = window.matchMedia ? window.matchMedia.bind(window) : null;
      // Replace matchMedia before any page script reads it
      window.matchMedia = query => {
        const s = String(query).toLowerCase(); let r = null;
        const f = val => { if (r !== false) r = val; }; let m;
        if ((m = s.match(/min-width:\s*([\d.]+)px/))) f(ew >= parseFloat(m[1]));
        if ((m = s.match(/max-width:\s*([\d.]+)px/))) f(ew <= parseFloat(m[1]));
  ...
  applyVMSpoof(); // must run before any page JS
  ...
  const swG   = (mod,pairs) => `<div class="gr">${pairs.map(([l,k])=>`<div class="r"><span>${esc(l)}</span><label class="sw"><input type="checkbox" data-sw="${mod}.${k}"${C[mod][k]?" checked":""}><span class="tk"></span></label></div>`).join("")}</div>`;
  ...
  const Panel = (() => {
    ...
    function render() {
      ...
      safeSetHTML(sh, `<style>${PCSS}</style><div class="bk" data-x></div><div class="cd" role="dialog"><div class="hd"><h1>🧼 Web Cleaner</h1><button class="x" data-x>✕</button></div>${secSB()}${secVM()}${isFB?secFB():""}${isYT?secYT():""}</div>`);
      ...
      qa("[data-ab]",sh).forEach(btn=>{
        ...
        const go=()=>{
          ...
          edit(m,()=>{
            ...
            if(m==="siteBlocker"){ if(k==="custom") C.siteBlocker.allow=C.siteBlocker.allow.filter(d=>d!==v); if(k==="allow") C.siteBlocker.custom=C.siteBlocker.custom.filter(d=>d!==v); }
          });
        };
  ...
  function initCluster() {
    ...
    let sx=0,sy=0,drag=false,aptr=null;

    cl.addEventListener("pointerdown",e=>{ sx=e.clientX; sy=e.clientY; drag=false; aptr=e.pointerId; });
    cl.addEventListener("pointermove",e=>{ if(aptr===null) return; if(!drag&&Math.hypot(e.clientX-sx,e.clientY-sy)>6){ drag=true; try{cl.setPointerCapture(e.pointerId);}catch(_){} } if(drag){ e.preventDefault(); cl.style.left=clamp(e.clientX-19,0,innerWidth-38)+"px"; cl.style.top=clamp(e.clientY-19,0,innerHeight-38)+"px"; cl.style.right=cl.style.bottom="auto"; } });
    cl.addEventListener("pointerup",e=>{ if(drag){ try{cl.releasePointerCapture(e.pointerId);}catch(_){} try{localStorage.setItem(PFX+"clpos",JSON.stringify({l:parseInt(cl.style.left),t:parseInt(cl.style.top)}));}catch(_){} } aptr=null; drag=false; });
    cl.addEventListener("pointercancel",()=>{ aptr=null; drag=false; });

    // 🧼 always visible — opens panel on every site
    addBtn("🧼","#1c1c2e","#fff",".8","").addEventListener("click",()=>{ if(!drag) Panel.open(); });

    // 🖥/📱 view mode — always visible
    if (C.viewMode.showButton) {
      const icon=vmMode==="desktop"?"🖥":vmMode==="mobile"?"📱":"🔄";
      const vb=addBtn(icon,"rgba(20,20,34,.8)","#fff",".7",""); let lt=null;
      vb.addEventListener("pointerdown",e=>{ e.stopPropagation(); lt=setTimeout(()=>{ lt=null; setVM("auto"); },C.viewMode.longPressMs); });
      vb.addEventListener("pointerup",  ()=>{ if(lt){clearTimeout(lt);lt=null;if(!drag) setVM(vmMode==="desktop"?"mobile":"desktop");} });
      vb.addEventListener("pointercancel",()=>{ if(lt){clearTimeout(lt);lt=null;} });
      vb.addEventListener("pointermove",e=>e.stopPropagation());
    }

    // site-specific buttons
    if (isFB&&C.facebook.showToggleButton) addBtn("🧹","#fff","#111",C.facebook.enabled?"1":".3","fcf-btn").addEventListener("click",()=>{ if(!drag) toggleFB(!C.facebook.enabled); });
    if (isYT&&C.youtube.showToggleButton)  addBtn("⏭","#fff","#111",C.youtube.enabled?"1":".3","yt-btn").addEventListener("click",()=>{ if(!drag) toggleYT(!C.youtube.enabled); });
  ...
  // ── facebook ───────────────────────────────────────────────
  function toggleFB(on) { C.facebook.enabled=on; save("facebook"); document.documentElement.classList.toggle("fcf-off",!on); const b=document.getElementById("fcf-btn"); if(b) b.style.opacity=on?"1":".3"; }
  ...
  function initFB() {
    ...
    const STRIP=/[\u200b-\u200f\u202a-\u202e\ufeff\u00ad\u2060]/g;
    (()=>{ const R=["html:not(.fcf-off) [data-fcf]{display:none!important}"]; if(!isMFB){ const X="html.fcf-s:not(.fcf-off) "; if(f.hideRightSidebar)R.push(`${X}[role="complementary"]{display:none!important}`); if(f.hideLeftSidebar)R.push(`${X}[role="navigation"][aria-label="Shortcuts"]{display:none!important}`,`html:not(.fcf-off) [data-fcf-ln]{display:none!important}`); if(f.hideComposer)R.push(`${X}[role="region"][aria-label="Create a post"]{display:none!important}`); if(f.hideTopBar)R.push(`${X}[role="banner"],${X}[role="navigation"][aria-label="Facebook"],${X}[role="navigation"][aria-label="Account Controls and Settings"]{display:none!important}`); if(f.hideReelsTrays)R.push(`${X}[aria-label="Stories"],${X}[aria-label="Reels"]{display:none!important}`); R.push(`${X}[role="main"]{margin-left:auto!important;margin-right:auto!important}`); if(f.hideTopBar)R.push(`${X}body{padding-top:0!important}`); } addStyle("fcf-css",R.join("\n")); })();
    function readText(scope,bt,bb){ const g=[]; let budget=600; const walker=document.createTreeWalker(scope,NodeFilter.SHOW_TEXT,null); let n; while((n=walker.nextNode())&&budget-->0){ const s=n.nodeValue; if(!s?.trim()) continue; const p=n.parentElement; if(!p||p.closest('[aria-hidden="true"]')) continue; const cs=getComputedStyle(p); if(cs.display==="none"||cs.visibility==="hidden"||cs.opacity==="0"||cs.fontSize==="0px") continue; const pr=p.getBoundingClientRect(); if(!pr.width||!pr.height||pr.right<=0||pr.bottom<bt||pr.top>bb) continue; const rng=document.createRange(); rng.selectNodeContents(n); const r=rng.getBoundingClientRect(); if(!r.width||!r.height||r.right<=0||r.top<bt||r.top>bb) continue; g.push({c:s.trim(),t:Math.round(r.top),l:Math.round(r.left)}); } const seen=new Set(),kept=[]; for(const x of g){ const k=`${x.t}:${x.l}`; if(!seen.has(k)){seen.add(k);kept.push(x);} } kept.sort((a,b)=>(a.t-b.t)||(a.l-b.l)); return kept.map(x=>x.c).join("").replace(STRIP,"").replace(/\s+/g," ").trim(); }
    const isJunk=c=>MARKS.some(m=>c.includes(m))||EXACT.includes(c);
    let _feed=null,_skipEl=null,_skipN=0; interceptNav(()=>{ _feed=null;_skipEl=null;_skipN=0; });
    function feedBox(){ if(_feed?.isConnected) return _feed; const main=q('[role="main"]'); if(!main) return null; let best=null,bn=1; for(const d of main.querySelectorAll("div")){ let n=0; for(const c of d.children){ const r=c.getBoundingClientRect(); if(r.width>=500&&r.width<=720&&r.height>60) n++; } if(n>bn){bn=n;best=d;} } return(_feed=best); }
    function processDesktop(){ const fd=feedBox(); if(!fd) return; const vh=innerHeight; for(const st of fd.children){ if(st._wc==="h"||st._wc==="c") continue; const r=st.getBoundingClientRect(); if(r.height<60||r.bottom<-500||r.top>vh+500) continue; const hdr=readText(st,r.top-2,r.top+130); if(!hdr) continue; if(isJunk(norm(hdr))||(f.hideReelsTrays&&st.querySelectorAll('a[href*="/reel/"]').length>3)){st.setAttribute("data-fcf","");st._wc="h";}else if((st._wcN=(st._wcN||0)+1)>=4) st._wc="c"; } }
    function processMobile(){ for(const p of document.querySelectorAll("[data-tracking-duration-id]")){ if(p._wc) continue; let junk=false; for(const e of p.querySelectorAll('span,a[role="link"],h3,h4,div[role="heading"]')){ if(junk) break; const raw=(e.textContent||"").trim(); if(!raw||raw.length>40) continue; const t=norm(raw); if(!t) continue; if(MARKS.some(m=>t===m||t.startsWith(m))||EXACT.includes(t)) junk=true; } if(junk){p.setAttribute("data-fcf","");p._wc="h";}else if((p._wcN=(p._wcN||0)+1)>=4) p._wc="c"; } }
    function hideLeftNav(){ if(!f.hideLeftSidebar) return; for(const n of document.querySelectorAll('[role="navigation"]:not([data-fcf-ln])')){ const r=n.getBoundingClientRect(); if(r.height>350&&r.width>=120&&r.width<=460&&r.left<=24) n.setAttribute("data-fcf-ln",""); } }
    const _reelSt=new WeakMap(); let _skipT=0;
    function handleReels(){ if(!f.skipReelsAds||!/^\/reels?(\/|$)/.test(location.pathname)) return; const cy=innerHeight/2; let act=null,best=1e9; for(const v of document.querySelectorAll("video")){ const r=v.getBoundingClientRect(); if(r.height<200) continue; const d=Math.abs((r.top+r.bottom)/2-cy); if(d<best){best=d;act=v;} } if(!act) return; let rl=act; for(let i=0;i<12&&rl.parentElement;i++){rl=rl.parentElement;if(rl.querySelector('[aria-label="Like"],[aria-label^="Comment"],[role="button"][aria-label="Next Card"]'))break;} if(!reelSpon(rl,act)){if(act!==_skipEl){_skipEl=null;_skipN=0;}return;} if(act!==_skipEl){_skipEl=act;_skipN=0;} if(Date.now()-_skipT<600||_skipN>=8) return; _skipN++;_skipT=Date.now(); const nx=q('[role="button"][aria-label="Next Card"]'); if(nx){nx.click();return;} const tg=rl.closest("[tabindex]")||rl; for(const tp of["keydown","keyup"]) tg.dispatchEvent(new KeyboardEvent(tp,{key:"ArrowDown",code:"ArrowDown",keyCode:40,which:40,bubbles:true})); }
    function reelSpon(rl,key){ let st=_reelSt.get(key); if(!st) _reelSt.set(key,(st={s:false,n:0})); if(st.s) return true; if(st.n>=8) return false; st.n++; const r=rl.getBoundingClientRect(),c=norm(readText(rl,r.top-2,r.bottom+2)); if(SPON.some(m=>c.includes(m))) st.s=true; return st.s; }
    const TKEYS=new Set("fbclid gclid dclid gbraid wbraid msclkid yclid twclid igshid mc_eid mc_cid _openstat vero_id oly_enc_id oly_anon_id wickedid _hsenc _hsmi mkt_tok ref refsrc refid fref hc_ref hc_location ref_src ref_url eav paipv comment_tracking av rdid".split(" ")), SHIMS=new Set(["l.facebook.com","lm.facebook.com","l.messenger.com"]), isTK=k=>TKEYS.has(k)||k.startsWith("utm_")||k.startsWith("__");
    ...
    const isFeed=()=>{ const pp=location.pathname; return pp==="/"||pp==="/home.php"; }, isClean=()=>{ const pp=location.pathname.replace(/\/$/,""); return isFeed()||pp==="/groups/feed"||pp==="/watch"||/^\/groups\/[^/]+$/.test(pp); };
    function sweep(){ try{ if(f.stripTracking) cleanLinks(); if(isMFB){processMobile();return;} hideLeftNav(); document.documentElement.classList.toggle("fcf-s",isFeed()); if(isClean()) processDesktop(); handleReels(); }catch(_){} }
    ...
    onReady(()=>{ sweep(); new MutationObserver(schedule).observe(document.documentElement,{childList:true,subtree:true}); window.addEventListener("scroll",schedule,{passive:true}); setInterval(sweep,isMFB?1000:1500); });
    onHotkey(()=>f.toggleHotkey,()=>toggleFB(!C.facebook.enabled));
  }

  // ── youtube ────────────────────────────────────────────────
  function toggleYT(on) { C.youtube.enabled=on; save("youtube"); const s=document.getElementById("yt-css"); if(s) s.disabled=!on; const b=document.getElementById("yt-btn"); if(b) b.style.opacity=on?"1":".3"; }
