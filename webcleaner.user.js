...
(function () {
  ...
  function initFB(){
    ...
    document.documentElement.classList.toggle("fcf-s",isFeed());
    onReady(()=>{sweep();new MutationObserver(schedule).observe(document.documentElement,{childList:true,subtree:true});window.addEventListener("scroll",schedule,{passive:true});setInterval(sweep,1200);});
  ...
  function initYT(){
    ...
    if(y.hideMerch)rules.push("ytd-merch-shelf-renderer","ytd-ticket-shelf-renderer","#donation-shelf","ytmusic-mealbar-promo-renderer","ytd-mealbar-promo-renderer","yt-mealbar-promo-renderer","tp-yt-paper-dialog:has(ytd-enforcement-message-view-model)");
    ...
    function tick(){
      ...
      try{
        ...
        if(y.skipVideoAds){
          ...
          const adState=(()=>{try{return typeof pl?.getAdState==="function"?pl.getAdState():-1;}catch(_){return -1;}})();
          const adActive=(hasC(pl,AD_V)&&adUiPresent)||adState>0;
        ...
        if(y.skipShortsAds&&/^\/shorts/.test(location.pathname)){
          ...
          const spState=(()=>{try{return typeof sp?.getAdState==="function"?sp.getAdState():-1;}catch(_){return -1;}})();
          const adOn=(hasC(sp,AD_S)&&shortsAdUi)||spState>0||!!q("ytd-reel-video-renderer ad-slot-renderer,ytd-reel-video-renderer ytd-ad-slot-renderer,ytd-shorts ytd-ad-slot-renderer,ytd-shorts ad-slot-renderer,ytd-reel-player-renderer ad-slot-renderer");
